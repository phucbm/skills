...
//
// THEME_NAME (optional) — constant defined in functions.php.
// Sets the block category label shown in the Gutenberg inserter.
// Defaults to "Project" if not defined.
//
// THEME_ASSETS_HANDLE (optional) — prefix for generated script handles.
// Defaults to "theme".
// ---------------------------------------------------------------------------
...
function px_block_categories(array $categories): array{
	$project_name  = defined('THEME_NAME') ? THEME_NAME : 'Project';
...
function px_override_block_view_scripts($metadata){
	...
	foreach($view_scripts as $script){
		...
		if(file_exists($script_file)){
			...
			$handle_prefix = defined('THEME_ASSETS_HANDLE') ? THEME_ASSETS_HANDLE : 'theme';
